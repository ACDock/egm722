...
join['Pop_dens'] = join['Population'] / join['Area_SqKM']
print(join.groupby(['CountyName'])['Pop_dens'].sum())
...
print('Population in original wards file: ', ward_popl)
print('Population in clipped file: ', clipped_ward_popl)

ward_popdens_max = join[['Ward', 'Pop_dens']][join.Pop_dens == join['Pop_dens'].max()]
ward_popdens_min = join[['Ward', 'Pop_dens']][join.Pop_dens == join['Pop_dens'].min()]
print('The Ward highest population density is:', ward_popdens_max)
print('The Ward lowest population density is:', ward_popdens_min)
...
# save the figure
fig.savefig('Prac3a_map.png', dpi=300, bbox_inches='tight')
